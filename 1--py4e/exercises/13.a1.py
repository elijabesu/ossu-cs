...
# Actual data: http://py4e-data.dr-chuck.net/comments_925878.xml (Sum ends with 15)
# 
...
# of the XML down to the comments node and then loop through the child
...
for comment in comments:
    total1 += int(comment.find("count").text)

print("Retrieved " + str(len(data.decode())) + " characters.")
print("Count:", len(comments))
print("Sum:", total1)

# the way they are suggesting:
counts = tree.findall('.//count')
total2 = 0
for count in counts:
    total2 += int(count.text)
print(total2)